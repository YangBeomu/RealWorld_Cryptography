...
static OSSL_LIB_CTX* osslLibCtx = NULL;
...
GCM_Ctx AES_GCM_Encrypt(uint8_t* data, size_t data_size, uint8_t* aad, size_t aad_size) {
	static GCM_Ctx gcmCtx{};

	//gcmCtx.dataPtr_ = shared_ptr<uint8_t[]>(new uint8_t(data_size));
	//memcpy_s(gcmCtx.dataPtr_.get(), data_size, data, data_size);

	/*gcmCtx.aad_ = shared_ptr<uint8_t[]>(new uint8_t(aad_size));
	memcpy_s(gcmCtx.aad_.get(), aad_size, aad, aad_size);*/
	...
	try {
		...
		if (!RAND_bytes(gcmCtx.gcmIV_, GCM_Ctx::IV_SIZE)) throw runtime_error("Failed to call RAND_bytes");
		...
		if (!RAND_bytes(gcmCtx.aesKey_, GCM_Ctx::KEY_SIZE)) throw runtime_error("Failed to call RAND_bytes");
		...
		//if (!EVP_EncryptUpdate(eCipherCtx, NULL, reinterpret_cast<int*>(&gcmCtx.dataSize_), gcmCtx.aad_.get(), gcmCtx.aadSize_)) throw runtime_error("Failed to call EVP_EncryptUpdate");
		...
		//if (!EVP_EncryptUpdate(eCipherCtx, gcmCtx.dataPtr_.get(), reinterpret_cast<int*>(&gcmCtx.dataSize_), data, data_size)) throw runtime_error("Failed to call EVP_EncryptUpdate");
		...
		cout << "[ENCRYPTED] ";
		//BIO_dump_fp(stdout, gcmCtx.dataPtr_.get(), gcmCtx.dataSize_);
		BIO_dump_fp(stdout, gcmCtx.dataPtr_, gcmCtx.dataSize_);

		uint8_t out[1024]{};
		int outLen = 0;

		//if (!EVP_EncryptFinal_ex(eCipherCtx, gcmCtx.dataPtr_.get(), reinterpret_cast<int*>(&gcmCtx.dataSize_))) throw runtime_error("Failed to call EVP_EncryptFinal_ex");
...
//	EVP_CIPHER* cipher = NULL;
//
//	shared_ptr<uint8_t[]> outPtr(new uint8_t(gcmCtx.dataSize_));
//	int outLen{};
//
//	try {
//		//encrypt
//		cout << "=== AES GCM Decrypt === " << endl;
//		cout << "[PLANE_TEXT] "; BIO_dump_fp(stdout, gcmCtx.dataPtr_.get(), gcmCtx.dataSize_);
//
//		/*osslLibCtx = OSSL_LIB_CTX_new();
//		if (osslLibCtx == NULL) throw runtime_error("Failed to call OSSL_LIB_CTX_new");*/
//
...
//		//memcpy_s(gcmCtx.aad_.get(), aad_size, aad, aad_size);
//
//		if (!EVP_DecryptUpdate(eCipherCtx, NULL, &outLen, gcmCtx.aad_.get(), gcmCtx.aadSize_)) throw runtime_error("Failed to call EVP_DecryptUpdate");
//		if (!EVP_DecryptUpdate(eCipherCtx, outPtr.get(), &outLen, gcmCtx.dataPtr_.get(), gcmCtx.dataSize_)) throw runtime_error("Failed to call EVP_DecryptUpdate");
//
//		cout << "[DECRYPTED] ";
//		BIO_dump_fp(stdout, outPtr.get(), outLen);
//
//		params[0] = OSSL_PARAM_construct_octet_string(OSSL_CIPHER_PARAM_AEAD_TAG, gcmCtx.tag_, GCM_Ctx::TAG_SIZE);
//		if (!EVP_CIPHER_CTX_set_params(eCipherCtx, params)) throw runtime_error("Failed to call EVP_CIPHER_CTX_set_params");
//
//		int ret = EVP_DecryptFinal_ex(eCipherCtx, outPtr.get(), &outLen);
//
...
int main(int argc, char* argv[]) {
	...
	try {
		...
		/*shared_ptr<uint8_t[]> dataPtr = AES_GCM_Decrypt(ctx);
		if (dataPtr == NULL) throw runtime_error("Failed to call AES_GCM_Decrypt");*/
