...
bool NetworkController::Open() {
	...
	try {
		...
		if (listen(socket_, 1) == SOCKET_ERROR)
			throw runtime_error("Failed to call listen");

		sockaddr_in clientInfo{};
		int addrLen = sizeof(clientInfo);

		if ((clientSocket_ = accept(socket_, reinterpret_cast<sockaddr*>(&clientInfo), &addrLen)) == INVALID_SOCKET)
...
bool NetworkController::Connect() {
	...
	try {
		...
		if (connect(socket_, reinterpret_cast<sockaddr*>(&sockAddr), sizeof(sockAddr)) == SOCKET_ERROR)
...
NetworkController::KeyData NetworkController::Recv() {
	...
	try {
		...
		/*if (recv(clientSocket_, reinterpret_cast<char*>(&kd.dataLen_), sizeof(kd.dataLen_), 0) <= 0)
			throw runtime_error("Failed call recv");
		if (recv(clientSocket_, reinterpret_cast<char*>(kd.dataPtr_), kd.dataLen_, 0) <= 0)
			throw runtime_error("Failed call recv");*/

		if (recv(clientSocket_, reinterpret_cast<char*>(test), 1024, 0) <= 0)
			throw runtime_error("Failed call recv");

		memcpy(&kd.dataLen_, test, sizeof(kd.dataLen_));
		kd.dataPtr_ = new uint8_t[kd.dataLen_];
		memcpy(kd.dataPtr_, test + sizeof(kd.dataLen_), kd.dataLen_);
