...
EVP_PKEY* GetParseKey(const NetworkController::KeyData kd) {
	...
	try {
		mem = BIO_new_mem_buf(kd.dataPtr_, kd.dataLen_);
...
void SaveKey(HANDLE publicFile, HANDLE privateFile) {
	...
	try {
		...
		if (EVP_PKEY_paramgen(pCtx, &params) != 1) throw runtime_error("Failed to call EVP_PKEY_paramgen");
...
int main(int argc,char* argv[]) {
	...
	size_t keyLen{};
	uint8_t* sKeyPtr = ECDH(nc.Recv(), &keyLen);
	if (sKeyPtr == nullptr) return -1;

	cout << "=== Private Key ===" << endl;
	for (int i = 0; i < keyLen; i++)
		printf("%02x", sKeyPtr[i]);
	cout << endl;
	cout << "===================" << endl;
}