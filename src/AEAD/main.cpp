...
struct GCM_Ctx {
	...
	bool isEmpty() {
		if (this->dataPtr_ == nullptr) return true;
		else return false;
	}
};

static OSSL_LIB_CTX* osslLibCtx = nullptr;
...
GCM_Ctx AES_GCM_Encrypt(uint8_t* data, size_t data_size, uint8_t* aad, size_t aad_size) {
	...
	gcmCtx.dataSize_ = data_size;

	/*OSSL_LIB_CTX* osslLibCtx = nullptr;*/
	EVP_CIPHER_CTX* eCipherCtx = nullptr;
	...
	try {
		...
		static const char* props = nullptr;
...
int main(int argc, char* argv[]) {
	...
	try {
		...
		if (dataPtr == nullptr) throw runtime_error("Failed to call AES_GCM_Decrypt");
